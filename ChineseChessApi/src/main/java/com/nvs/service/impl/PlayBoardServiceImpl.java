...
public class PlayBoardServiceImpl implements PlayBoardService{
   ...
   public PlayBoardDTO generate(){
      ...
      pieces.stream().map(pieceMapper::toDTO).forEach(pDTO->playBoardDTO.getState()[pDTO.getCurrentCol()][pDTO.getCurrentRow()] = pDTO);
   ...
   public PlayBoardDTO build(List<MoveHistory> moveHistories){
      return moveHistories.stream().reduce(generate(), (playBoardDTO, mh)->update(playBoardDTO, pieceService.findOneInBoard(playBoardDTO, mh.getPiece().getId()), mh.getToCol(), mh.getToRow()), (playBoardDTO, updatedBoardDTO)->updatedBoardDTO);
   }
   ...
   public boolean isGeneralBeingChecked(PlayBoardDTO playBoardDTO, PieceDTO generalPieceDTO){
      List<PieceDTO> opponentPieceDTOsInBoard = pieceService.findAllInBoard(playBoardDTO, null, !generalPieceDTO.isRed());

      return opponentPieceDTOsInBoard.stream().anyMatch(opponentPiece->moveRuleService.isValid(playBoardDTO, opponentPiece, generalPieceDTO.getCurrentCol(), generalPieceDTO.getCurrentRow()));
   }
   ...
   public boolean isGeneralInSafe(PlayBoardDTO playBoardDTO, PieceDTO generalPieceDTO){
      ...
      return (!areTwoGeneralsFacing(playBoardDTO, generalPieceDTO, opponentGeneralDTO) && !isGeneralBeingChecked(playBoardDTO, generalPieceDTO));
   }

   @Override
   public int evaluate(PlayBoardDTO playBoardDTO){
      return IntStream.rangeClosed(COL_MIN, COL_MAX).flatMap(col->IntStream.rangeClosed(ROW_MIN, ROW_MAX).filter(row->playBoardDTO.getState()[col][row] != null).map(row->{
   ...
   public void printTest(PlayBoardDTO playBoardDTO, PieceDTO pieceDTO, List<int[]> availableMoveIndexes){
      ...
      for(int row = ROW_MIN; row <= ROW_MAX; row++){
         for(int col = COL_MIN; col <= COL_MAX; col++){
            ...
            boolean containsIndex = availableMoveIndexes.stream().anyMatch(arr->Arrays.equals(arr, index));
            if(containsIndex){
   ...
   private String getSymbolOutput(PieceDTO movingPieceDTO, int col, int row, PieceDTO targetPieceDTO, boolean isValidMoveFinding){
      if(targetPieceDTO == null){
         if((movingPieceDTO != null) && ((col == movingPieceDTO.getCurrentCol()) && (row == movingPieceDTO.getCurrentRow()))){
            return "   [ ]   ";
         ...
         String symbol = this.formatNameSymbol(pieceService.convertByName(targetPieceDTO.getName()).getShortName(), targetPieceDTO.isRed(), targetPieceDTO.getId());

         if((movingPieceDTO != null) && (Objects.equals(targetPieceDTO.getId(), movingPieceDTO.getId()))){
            return "[" + symbol + "]";
         } else if(isValidMoveFinding){
            return "(" + symbol + ")";
         } else {
   ...
   private String formatNameSymbol(String shortName, boolean isRed, int id){
      return ((shortName.length() == 1 ? " " + shortName : shortName) + this.formatColorSymbol(isRed) + "_" + (shortName.length() == 1 ? this.formatIdSymbol(id) + " " : this.formatIdSymbol(id)));
   }
