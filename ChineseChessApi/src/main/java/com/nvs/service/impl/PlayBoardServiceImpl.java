...
public class PlayBoardServiceImpl implements PlayBoardService{
   ...
   public PlayBoardDTO build(List<MoveHistory> moveHistories){
      return moveHistories.stream()
                          .reduce(generate(), (playBoardDTO, mh)->update(playBoardDTO, pieceService.findOneInBoard(playBoardDTO, mh.getPiece()
                                                                                                                                   .getId()), mh.getToCol(), mh.getToRow()), (playBoardDTO, updatedBoardDTO)->updatedBoardDTO);
   }
   ...
   @Override
   public PlayBoardDTO restore(PlayBoardDTO playBoardDTO, PieceDTO movedPieceDTO, int fromCol, int fromRow, PieceDTO deadPieceDTO){
   ...
   public boolean isGeneralBeingChecked(PlayBoardDTO playBoardDTO, PieceDTO generalPieceDTO){
      ...
      return opponentPieceDTOsInBoard.stream()
                                     .anyMatch(opponentPiece->moveRuleService.isValid(playBoardDTO, opponentPiece, generalPieceDTO.getCurrentCol(), generalPieceDTO.getCurrentRow()));
   }
      ...
      return IntStream.rangeClosed(COL_MIN, COL_MAX)
                      .flatMap(col->IntStream.rangeClosed(ROW_MIN, ROW_MAX)
                                             ...
                                             .map(row->{
                                                ...
                                                int piecePower = pieceService.convertByName(pieceDTO.getName())
                                                                             .getPower();
   ...
   private String getSymbolOutput(PieceDTO movingPieceDTO, int col, int row, PieceDTO targetPieceDTO, boolean isValidMoveFinding){
      if(targetPieceDTO == null){
         ...
         String symbol = this.formatNameSymbol(pieceService.convertByName(targetPieceDTO.getName())
                                                           .getShortName(), targetPieceDTO.isRed(), targetPieceDTO.getId());
