...
public class MatchServiceImpl implements MatchService{
   ...
   public MatchDTO findById(long id){
      return matchRepository.findById(id).map(matchMapper::toDTO).orElseThrow(()->new ResourceNotFoundExceptionCustomize(Collections.singletonMap("id", id)));
   }

   @Override
   public List<MatchDTO> findAllByPlayerId(long playerId){
      return matchRepository.findAllByPlayerId(playerId).stream().map(matchMapper::toDTO).collect(Collectors.toList());
   }

   @Override
   public MatchDetailDTO findDetailById(long id){
      MatchDetailDTO matchDetailDTO = matchRepository.findById(id).map(matchMapper::toDetailDTO).orElseThrow(()->new ResourceNotFoundExceptionCustomize(Collections.singletonMap("id", id)));
   ...
   public MatchDTO create(MatchCreationDTO matchCreationDTO){
      Player player1 = playerRepository.findById(matchCreationDTO.getPlayer1Id()).orElseThrow(()->new ResourceNotFoundExceptionCustomize(Collections.singletonMap("player1Id", matchCreationDTO.getPlayer1Id())));

      Player player2 = playerRepository.findById(matchCreationDTO.getPlayer2Id()).orElseThrow(()->new ResourceNotFoundExceptionCustomize(Collections.singletonMap("player2Id", matchCreationDTO.getPlayer2Id())));
   ...
   public MatchDTO updateResult(long id, Boolean result){
      Match existingMatch = matchRepository.findById(id).orElseThrow(()->new ResourceNotFoundExceptionCustomize(Collections.singletonMap("id", id)));
      ...
      if(result == null){
         ...
         player1ProfileDTO = playerService.updateByMatchResult(existingMatch.getPlayer1().getId(), EMatchResult.DRAW.getValue(), existingMatch.getEloBet());
         player2ProfileDTO = playerService.updateByMatchResult(existingMatch.getPlayer2().getId(), EMatchResult.DRAW.getValue(), existingMatch.getEloBet());
      } else if(result){
         existingMatch.setResult(EMatchResult.WIN.getValue());
         player1ProfileDTO = playerService.updateByMatchResult(existingMatch.getPlayer1().getId(), EMatchResult.WIN.getValue(), existingMatch.getEloBet());
         player2ProfileDTO = playerService.updateByMatchResult(existingMatch.getPlayer2().getId(), EMatchResult.LOSE.getValue(), existingMatch.getEloBet());
      } else {
         existingMatch.setResult(EMatchResult.LOSE.getValue());
         player1ProfileDTO = playerService.updateByMatchResult(existingMatch.getPlayer1().getId(), EMatchResult.LOSE.getValue(), existingMatch.getEloBet());
         player2ProfileDTO = playerService.updateByMatchResult(existingMatch.getPlayer2().getId(), EMatchResult.WIN.getValue(), existingMatch.getEloBet());
      }
