...
public class PieceServiceImpl implements PieceService{
   ...
   public PieceDTO findById(int id){
      return pieceRepository.findById(id).map(pieceMapper::toDTO).orElseThrow(()->new ResourceNotFoundExceptionCustomize(Collections.singletonMap("id", id)));
   }
   ...
   @Override
   public List<PieceDTO> findAllInBoard(PlayBoardDTO playBoardDTO, String name, Boolean isRed, int fromCol, int fromRow, int toCol, int toRow){
      return IntStream.rangeClosed(fromCol, toCol).boxed().flatMap(col->IntStream.rangeClosed(fromRow, toRow).filter(row->{
         PieceDTO pieceDTO = playBoardDTO.getState()[col][row];

         return ((pieceDTO != null) && (StringUtils.isBlank(name) || pieceDTO.getName().equals(name)) && ((isRed == null) || (pieceDTO.isRed() == isRed)));
      }).mapToObj(row->playBoardDTO.getState()[col][row])).toList();
   }
   ...
   public List<PieceDTO> findAllNotInPlayBoard(PlayBoardDTO playBoardDTO){
      ...
      deadPieces.removeIf(deadPiece->piecesInBoard.stream().map(PieceDTO::getId).toList().contains(deadPiece.getId()));
   ...
   public PieceDTO findOneInBoard(PlayBoardDTO playBoardDTO, int id){
      ...
      return IntStream.rangeClosed(fromCol, toCol).boxed().flatMap(col->IntStream.rangeClosed(fromRow, toRow).filter(row->(playBoardDTO.getState()[col][row] != null) && (playBoardDTO.getState()[col][row].getId() == id)).mapToObj(row->playBoardDTO.getState()[col][row])).findFirst().orElse(null);
   }
   ...
   public PieceDTO findExistingTheSameInColPath(PlayBoardDTO playBoardDTO, PieceDTO pieceDTO){
      ...
      return IntStream.rangeClosed(fromRow, toRow).filter(row->{
         PieceDTO currentPiece = playBoardDTO.getState()[col][row];

         return ((currentPiece != null) && (!Objects.equals(currentPiece.getId(), pieceDTO.getId())) && (currentPiece.isRed() == pieceDTO.isRed()) && currentPiece.getName().equals(pieceDTO.getName()));
   ...
   public boolean existsBetweenInRowPath(PlayBoardDTO playBoardDTO, int currentRow, int fromCol, int toCol){
      ...
      return IntStream.rangeClosed(startCol, endCol).anyMatch(col->playBoardDTO.getState()[col][currentRow] != null);
   }
   ...
   public boolean existsBetweenInColPath(PlayBoardDTO playBoardDTO, int currentCol, int fromRow, int toRow){
      ...
      return IntStream.rangeClosed(startRow, endRow).anyMatch(row->playBoardDTO.getState()[currentCol][row] != null);
   }
   ...
   public int countBetweenInRowPath(PlayBoardDTO playBoardDTO, int currentRow, int fromCol, int toCol){
      ...
      return (int) IntStream.rangeClosed(startCol, endCol).filter(col->playBoardDTO.getState()[col][currentRow] != null).count();
   }
   ...
   public int countBetweenInColPath(PlayBoardDTO playBoardDTO, int currentCol, int fromRow, int toRow){
      ...
      return (int) IntStream.rangeClosed(startRow, endRow).filter(row->playBoardDTO.getState()[currentCol][row] != null).count();
   }
   ...
   public PieceDTO findGeneralInBoard(PlayBoardDTO playBoardDTO, boolean isRed){
      ...
      return findAllInBoard(playBoardDTO, EPiece.GENERAL.name(), null, fromCol, fromRow, toCol, toRow).stream().findFirst().orElse(null);
   }
