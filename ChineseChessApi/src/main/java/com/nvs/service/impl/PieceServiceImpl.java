...
public class PieceServiceImpl implements PieceService {
  ...
  public PieceDTO findExistingTheSameInColPath(PlayBoardDTO playBoardDTO, PieceDTO pieceDTO) {
    log.debug("Finding existing piece with same attributes in column path for piece: {}", pieceDTO);
  ...
  public int countBetweenInRowPath(PlayBoardDTO playBoardDTO, int currentRow, int fromCol,
      int toCol) {
    log.debug("Counting pieces between columns {} and {} in row {}.", fromCol, toCol, currentRow);
    int startCol = Math.min(fromCol, toCol) + 1;
    int endCol = Math.max(fromCol, toCol) - 1;
    ...
    log.debug("Count of pieces between columns {} and {} in row {}: {}", fromCol, toCol, currentRow,
        count);
    return count;
  }
  ...
  public int countBetweenInColPath(PlayBoardDTO playBoardDTO, int currentCol, int fromRow,
      int toRow) {
    log.debug("Counting pieces between rows {} and {} in column {}.", fromRow, toRow, currentCol);
    int startRow = Math.min(fromRow, toRow) + 1;
    int endRow = Math.max(fromRow, toRow) - 1;
    ...
    log.debug("Count of pieces between rows {} and {} in column {}: {}", fromRow, toRow, currentCol,
        count);
    return count;
  }
