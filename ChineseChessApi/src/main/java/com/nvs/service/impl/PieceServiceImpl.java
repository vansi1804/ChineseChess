...
public class PieceServiceImpl implements PieceService {
  ...
  public PieceDTO findById(int id) {
    return pieceRepository.findById(id).map(pieceMapper::toDTO).orElseThrow(
  ...
  public List<PieceDTO> findAllInBoard(PlayBoardDTO playBoardDTO, String name, Boolean isRed,
    ...
    return IntStream.rangeClosed(fromCol, toCol).boxed()
  ...
  public List<PieceDTO> findAllNotInPlayBoard(PlayBoardDTO playBoardDTO) {
    ...
    deadPieces.removeIf(deadPiece -> piecesInBoard.stream().map(PieceDTO::getId).toList()
  ...
  public PieceDTO findOneInBoard(PlayBoardDTO playBoardDTO, int id) {
    ...
    return IntStream.rangeClosed(fromCol, toCol).boxed().flatMap(
        col -> IntStream.rangeClosed(fromRow, toRow).filter(
                row -> (playBoardDTO.getState()[col][row] != null) && (
  ...
  public int countBetweenInRowPath(PlayBoardDTO playBoardDTO, int currentRow, int fromCol,
    ...
    return (int) IntStream.rangeClosed(startCol, endCol)
        .filter(col -> playBoardDTO.getState()[col][currentRow] != null).count();
  }
  ...
  public int countBetweenInColPath(PlayBoardDTO playBoardDTO, int currentCol, int fromRow,
    ...
    return (int) IntStream.rangeClosed(startRow, endRow)
        .filter(row -> playBoardDTO.getState()[currentCol][row] != null).count();
  }
  ...
  public PieceDTO findGeneralInBoard(PlayBoardDTO playBoardDTO, boolean isRed) {
    ...
    return findAllInBoard(playBoardDTO, EPiece.GENERAL.name(), null, fromCol, fromRow, toCol,
        toRow).stream().findFirst().orElse(null);
  }
