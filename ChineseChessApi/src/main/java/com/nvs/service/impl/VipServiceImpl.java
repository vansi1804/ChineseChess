...
public class VipServiceImpl implements VipService{
   ...
   public VipDTO findById(int id){
      return vipRepository.findById(id).map(vipMapper::toDTO).orElseThrow(()->new ResourceNotFoundExceptionCustomize(Collections.singletonMap("id", id)));
   }
   ...
   public VipDTO update(int id, VipDTO vipDTO){
      Vip existingVip = vipRepository.findById(id).orElseThrow(()->new ResourceNotFoundExceptionCustomize(Collections.singletonMap("id", id)));
   ...
   public boolean delete(int id){
      vipRepository.delete(vipRepository.findById(id).orElseThrow(()->new ResourceNotFoundExceptionCustomize(Collections.singletonMap("id", id))));
      return true;
   }
