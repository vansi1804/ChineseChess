...
public class MoveDescriptionServiceImpl implements MoveDescriptionService{
   ...
   public String build(PlayBoardDTO playBoardDTO, PieceDTO pieceDTO, int toCol, int toRow){
      ...
      int currentPosition = pieceDTO.isRed() ? (MAX_COL - pieceDTO.getCurrentCol() + 1) : pieceDTO.getCurrentCol() + 1;
      char moveType;
      int targetPosition;
      ...
      if(moveTypeService.isHorizontallyMoving(pieceDTO.getCurrentRow(), toRow)){
         ...
         moveType = moveTypeService.isUpMoving(pieceDTO.isRed(), pieceDTO.getCurrentRow(), toRow) ? EMoveTypeDescription.UP.getValue() : EMoveTypeDescription.DOWN.getValue();

         targetPosition = moveTypeService.isVerticallyMoving(pieceDTO.getCurrentCol(), toCol) ? Math.abs(pieceDTO.getCurrentRow() - toRow) : (pieceDTO.isRed() ? (MAX_COL - toCol + 1) : toCol + 1);
      }
   ...
   private Character buildIndexDescription(PlayBoardDTO playBoardDTO, PieceDTO pieceDTO){
      ...
      if(foundPiece == null){
         ...
         boolean isRowBefore = (pieceDTO.isRed() && (pieceDTO.getCurrentRow() < foundPiece.getCurrentRow())) || (!pieceDTO.isRed() && (pieceDTO.getCurrentRow() > foundPiece.getCurrentRow()));
