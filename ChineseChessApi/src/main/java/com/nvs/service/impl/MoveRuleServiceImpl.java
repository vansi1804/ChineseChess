...
public class MoveRuleServiceImpl implements MoveRuleService {
  ...
  public boolean isValid(PlayBoardDTO playBoardDTO, PieceDTO pieceDTO, int toCol, int toRow) {
    ...
    return switch (ePiece) {
      ...
      case ELEPHANT ->
          this.checkElephantMoveRule(playBoardDTO, pieceDTO.isRed(), fromCol, toCol, fromRow,
              toRow);
   ...
   */
  private boolean checkGeneralMoveRule(boolean isRed, int fromCol, int toCol, int fromRow,
      int toRow) {
    return (isInAreaCenter(isRed, toCol, toRow) && (
        (moveTypeService.isHorizontallyMoving(fromRow, toRow) && (Math.abs(toCol - fromCol) == 1))
            || (moveTypeService.isVerticallyMoving(fromCol, toCol) && (Math.abs(toRow - fromRow)
            == 1))));
  }
   ...
   */
  private boolean checkGuardMoveRule(boolean isRed, int fromCol, int toCol, int fromRow,
      int toRow) {
   ...
   */
  private boolean checkElephantMoveRule(PlayBoardDTO playBoardDTO, boolean isRed, int fromCol,
      int toCol, int fromRow, int toRow) {
    int verticalSpace = Math.abs(toRow - fromRow);
    int horizontalSpace = Math.abs(toCol - fromCol);
   ...
   */
  private boolean checkHorseMoveRule(PlayBoardDTO playBoardDTO, int fromCol, int toCol, int fromRow,
      int toRow) {
    int verticalSpace = Math.abs(toRow - fromRow);
    int horizontalSpace = Math.abs(toCol - fromCol);
   ...
   */
  private boolean checkChariotMoveRule(PlayBoardDTO playBoardDTO, int fromCol, int toCol,
      int fromRow, int toRow) {
    return (
        (moveTypeService.isVerticallyMoving(fromCol, toCol) && pieceService.existsBetweenInColPath(
            playBoardDTO, fromCol, fromRow, toRow)) || (
   ...
   */
  private boolean checkCannonMoveRule(PlayBoardDTO playBoardDTO, int fromCol, int toCol,
      int fromRow, int toRow) {
    PieceDTO targetPiece = playBoardDTO.getState()[toCol][toRow];
    ...
    if (isVerticallyMoving || isHorizontalMoving) {
      int numPiecesBetween =
          isVerticallyMoving ? pieceService.countBetweenInColPath(playBoardDTO, fromCol, fromRow,
              toRow) : pieceService.countBetweenInRowPath(playBoardDTO, fromRow, fromCol, toCol);
   ...
   */
  private boolean checkSoldierMoveRule(boolean isRed, int fromCol, int toCol, int fromRow,
      int toRow) {
    return ((moveTypeService.isVerticallyMoving(fromCol, toCol) && (isRed ? (fromRow - toRow == 1)
        : (toRow - fromRow == 1))) || (moveTypeService.isHorizontallyMoving(fromRow, toRow)
        && this.isOverTheRiver(isRed, fromRow) && (Math.abs(fromCol - toCol) == 1)));
  }

  private boolean isInAreaCenter(boolean isRed, int col, int row) {
    return (((CENTER_COL_MIN <= col) && (col <= CENTER_COL_MAX)) && (isRed ? (
