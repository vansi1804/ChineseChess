...
import com.data.mapper.LevelsMapper;
...
public class LevelsServiceImpl implements LevelsService {
    ...
    public LevelsDTO findById(int id) {
        return levelsRepository.findById(id).map(r -> levelsMapper.toDTO(r))
                .orElseThrow(() -> new ExceptionCustom("Levels", ErrorMessage.DATA_NOT_FOUND, "id", id));
    }

    @Override
    public LevelsDTO findByName(String name) {
        return levelsRepository.findByName(name).map(r -> levelsMapper.toDTO(r))
                .orElseThrow(() -> new ExceptionCustom("Levels", ErrorMessage.DATA_NOT_FOUND, "name", name));
    }

    @Override
    public LevelsDTO create(LevelsDTO levelsDTO) {
        if (levelsRepository.findByName(levelsDTO.getName()).isPresent()) {
            throw new ExceptionCustom("Levels", ErrorMessage.DATA_EXISTING, "name", levelsDTO.getName());
        }
    ...
    public void delete(int id) {
        levelsRepository.delete(levelsRepository.findById(id)
                .orElseThrow(() -> new ExceptionCustom("Levels", ErrorMessage.DATA_NOT_FOUND, "id", id)));
    }
